jQuery(document).ready(function( $ ) {
	...
	// Method for Control's event handlers: cs_pp_s_quantity_style_typography.
	wp.customize('cs_pp_s_quantity_style_typography', function (value) {
		var typography = mkPreviewTypography(value(), true);
		// calculate quantity input with based on border and font size.
		var quantity_input_width = parseInt( typography['font-size'], 10 ) + ( parseInt(wp.customize('cs_pp_s_quantity_style_border')(), 10)  * 2 );
		...
		value.bind(function (to) {
			...
			// calculate quantity input with based on border and font size.
			var quantity_input_width = parseInt( typography['font-size'], 10 ) + ( parseInt(wp.customize('cs_pp_s_quantity_style_border')(), 10) * 2 );
			parseInt( typography['font-size'], 10 ) + parseInt(wp.customize('cs_pp_s_quantity_style_border')(), 10)
			$( quantity_style_label + ', ' + quantity_style_input ).css(typography);
			$( quantity_style_input ).css( 'width', quantity_input_width + 'px' );
		});
	...
	// Method for Control's event handlers: cs_pp_s_quantity_style_bg_color.
	wp.customize( 'cs_pp_s_quantity_style_bg_color', function( value ) {
	...
	// Method for Control's event handlers: cs_pp_s_quantity_style_border.
	wp.customize('cs_pp_s_quantity_style_border', function (value) {
		...
		// calculate quantity input with based on border and font size.
		var typography = mkPreviewTypography( wp.customize('cs_pp_s_quantity_style_typography')() );
	...
	// Method for Control's event handlers: cs_pp_s_quantity_style_border_color.
	wp.customize('cs_pp_s_quantity_style_border_color', function (value) {
		$( quantity_style_input + ', ' + quantity_style_button ).css({
			'border-color': value()
		});
		value.bind(function (to) {
	...
	// Method for Control's event handlers: cs_pp_s_quantity_style_box_model.
	wp.customize('cs_pp_s_quantity_style_box_model', function (value) {
